...
class SSP {
    ...
    static function custom_complex ( $request, $conn, $table, $primaryKey, $columns, $whereResult='', $whereAll='', $group_by = '', $c_columns = '' )
    {
        $bindings = array();
        $db = SSP::sql_connect( $conn );
        $localWhereResult = array();
         ...
         * Output
         */
        if(isset($c_columns) && !empty($c_columns)){
            //$columns = $c_columns;
            $query = "SELECT `".implode("`, `", SSP::pluck($columns, 'db'))."`  FROM $table $where $group_by";
            $resFilterLength = SSP::sql_exec( $db, $bindings,
                "SELECT COUNT(*)
                 FROM   ($query) t"
            );
            $recordsFiltered = $resFilterLength[0][0];
            // Total data set length
            $query1 = "SELECT COUNT(`{$primaryKey}`) FROM   $table ". $whereAllSql ." $group_by";
            $resTotalLength = SSP::sql_exec( $db, $bindings,
                "SELECT COUNT(*)
                 FROM   ($query1) t"
            );
